...
import {
  getBcvVerifyStruct,
  isVerifiedWithBcvStruct,
} from "../helpers/bcvVerify";
...
export default function usePkBookPreviewRenderer(props) {
  const { pk, docId, bookId, renderStyles } = props;
  ...
  const doRender = useCallback(
    ({ renderFlags, htmlRender, verbose, extInfo }) => {
      const output = {};
      if (docId != null && pk != null && bookId != null) {
        ...
        let renderers = null;
        if (htmlRender) {
          // create a class instance of Render2Html and then get render functions bound to instance
